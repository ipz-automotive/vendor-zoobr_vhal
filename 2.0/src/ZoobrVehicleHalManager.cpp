...
Return<void> ZoobrVehicleHalManager::getAllPropConfigs(getAllPropConfigs_cb _hidl_cb) {
...
Return<void> ZoobrVehicleHalManager::getPropConfigs(const hidl_vec<int32_t> &properties,
...
Return<void> ZoobrVehicleHalManager::get(const VehiclePropValue& requestedPropValue, get_cb _hidl_cb) {
...
Return<StatusCode> ZoobrVehicleHalManager::set(const VehiclePropValue &value) {
...
Return<StatusCode> ZoobrVehicleHalManager::subscribe(const sp<IVehicleCallback> &callback,
...
Return<StatusCode> ZoobrVehicleHalManager::unsubscribe(const sp<IVehicleCallback>& callback,
                                                  int32_t propId) {
    mSubscriptionManager.unsubscribe(getClientId(callback), propId);
    return StatusCode::OK;
}

Return<void> ZoobrVehicleHalManager::debugDump(IVehicle::debugDump_cb _hidl_cb) {
    _hidl_cb("");
    return Void();
}

Return<void> ZoobrVehicleHalManager::debug(const hidl_handle& fd, const hidl_vec<hidl_string>& options) {
...
void ZoobrVehicleHalManager::cmdDump(int fd, const hidl_vec<hidl_string>& options) {
    if (options.size() == 0) {
...
bool ZoobrVehicleHalManager::checkCallerHasWritePermissions(int fd) {
...
bool ZoobrVehicleHalManager::checkArgumentsSize(int fd, const hidl_vec<hidl_string>& options,
                                           size_t minSize) {
...
template <typename T>
bool ZoobrVehicleHalManager::safelyParseInt(int fd, int index, const std::string& s, T* out) {
...
bool ZoobrVehicleHalManager::safelyParseFloat(int fd, int index, const std::string& s, float* out) {
...
void ZoobrVehicleHalManager::cmdHelp(int fd) const {
    dprintf(fd, "Usage: \n\n");
...
void ZoobrVehicleHalManager::cmdListAllProperties(int fd) const {
    auto& halConfig = mConfigIndex->getAllConfigs();
...
void ZoobrVehicleHalManager::cmdDumpAllProperties(int fd) {
    auto& halConfig = mConfigIndex->getAllConfigs();
    size_t size = halConfig.size();
    if (size == 0) {
...
void ZoobrVehicleHalManager::cmdDumpOneProperty(int fd, int rowNumber, const VehiclePropConfig& config) {
...
void ZoobrVehicleHalManager::cmdDumpSpecificProperties(int fd, const hidl_vec<hidl_string>& options) {
...
void ZoobrVehicleHalManager::cmdDumpOneProperty(int fd, int32_t prop, int32_t areaId) {
...
bool ZoobrVehicleHalManager::cmdSetOneProperty(int fd, const hidl_vec<hidl_string>& options) {
    ...
    // Do not use ZoobrVehicleHalManager::set here because we don't want to check write permission.
...
void ZoobrVehicleHalManager::init() {
    ALOGI("ZoobrVehicleHalManager::init");
    ...
    mBatchingConsumer.run(&mEventQueue,
                          ...
                          std::bind(&ZoobrVehicleHalManager::onBatchHalEvent,
                                    this, _1));

    mHal->init(&mValueObjectPool,
               std::bind(&ZoobrVehicleHalManager::onHalEvent, this, _1),
               std::bind(&ZoobrVehicleHalManager::onHalPropertySetError, this,
                         _1, _2, _3));
...
ZoobrVehicleHalManager::~ZoobrVehicleHalManager() {
    ...
    ALOGI("ZoobrVehicleHalManager::dtor");
}

void ZoobrVehicleHalManager::onHalEvent(VehiclePropValuePtr v) {
    mEventQueue.push(std::move(v));
}

void ZoobrVehicleHalManager::onHalPropertySetError(StatusCode errorCode,
                                              int32_t property,
                                              int32_t areaId) {
    const auto& clients =
...
void ZoobrVehicleHalManager::onBatchHalEvent(const std::vector<VehiclePropValuePtr>& values) {
...
bool ZoobrVehicleHalManager::isSampleRateFixed(VehiclePropertyChangeMode mode) {
    return (mode & VehiclePropertyChangeMode::ON_CHANGE);
}

float ZoobrVehicleHalManager::checkSampleRate(const VehiclePropConfig &config,
                                         float sampleRate) {
...
bool ZoobrVehicleHalManager::isSubscribable(const VehiclePropConfig& config,
...
bool ZoobrVehicleHalManager::checkWritePermission(const VehiclePropConfig &config) const {
...
bool ZoobrVehicleHalManager::checkReadPermission(const VehiclePropConfig &config) const {
...
void ZoobrVehicleHalManager::handlePropertySetEvent(const VehiclePropValue& value) {
    auto clients =
    ...
    // TODO: mMqttClient->publish();
}

const VehiclePropConfig* ZoobrVehicleHalManager::getPropConfigOrNull(
        int32_t prop) const {
    return mConfigIndex->hasConfig(prop)
           ? &mConfigIndex->getConfig(prop) : nullptr;
}

void ZoobrVehicleHalManager::onAllClientsUnsubscribed(int32_t propertyId) {
    mHal->unsubscribe(propertyId);
}

ClientId ZoobrVehicleHalManager::getClientId(const sp<IVehicleCallback>& callback) {
...
std::vector<std::string> ZoobrVehicleHalManager::getOptionValues(const hidl_vec<hidl_string>& options,
                                                            size_t* index) {
...
bool ZoobrVehicleHalManager::parseSetPropOptions(int fd, const hidl_vec<hidl_string>& options,
                                            VehiclePropValue* prop) {
    // Options format:
...
bool ZoobrVehicleHalManager::parseHexString(int fd, const std::string& s, std::vector<uint8_t>* bytes) {
    if (s.size() % 2 != 0) {
